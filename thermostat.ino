...
char* thermo_mode_2[] = { "chaud", "froid"};
//char* thermo_mode[] = {"off", "chaud", "froid"};
...
const char* ssid = "ALO_IoT";
const char* password = "homeiot3011";
...
void configure_screen() {
  ...
  attachInterrupt(digitalPinToInterrupt(SW), on_click, CHANGE);
...
void update_screen() {
  if (screen_on && (millis() - last_action) > 5000) {
    screen_on = false;
  }
  ...
  if (screen_on) {
    ...
    display.setTextColor(SSD1306_WHITE);        // Draw white text
    display.setCursor(0,0);             // Start at top-left corner
    display.print(thermo_mode_2[current_mode]);
    display.print(" ");
...
ICACHE_RAM_ATTR void updateEncoder(NewEncoder *encPtr, const volatile NewEncoder::EncoderState *state, void *uPtr){
  ...
  bool status = mqtt_client.publish(charTopic, charData, true);
  log_serial("Publish status: ");
  log_serialln(String(status));
}

ICACHE_RAM_ATTR void on_click() {
  if (updating) {
    return;
  }
  ...
  last_action = millis();
  screen_on = true;
  if (target_temp == "15" || target_temp == "15.0") {
    show_version = true;
  }
}
